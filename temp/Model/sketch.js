...
let loss;
const labels = [
  'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'L', 'O', 'P', 'V', 'W', 'Ok',
  'Y', 'Bad', 'I_love_you', 'Good', 'That', 'Hello'
];
const counts = {};
...
function setup() {
  ...
  video = createCapture(VIDEO);
  video.parent('videoContainer');
  video.size(w < 1000 ? 300 : 600, w < 1000 ? 300 : 500);
...
function setupButtons() {
  labels.forEach(label => {
    const button = select(`#${label}`);
    button.mousePressed(() => {
      classifier.addImage(label);
      counts[label]++;
      button.html(`${label}: ${counts[label]}`);
    });
  });
...
function gotResults(err, results) {
  ...
  if (results && results[0]) {
    ...
    if (confidence > 0.4) {
      ...
      select('#confidence').html(`${(confidence * 100).toFixed(2)}%`);
    }
    classify();
  }
}
